...
def _execute_fromimport_star(
    ...
    for name in imports:
        ...
        if hasattr(child_module, "__all__"):
            from_imports.append((name, child_module.__all__))
            continue
        ...
        for attrname in fromlist_:
            ...
            try:
                ...
                if not forced and attrval.__name__ in sys.modules:
                    if verbose > 1:
...
def _make_imports_str(imports, rootmodname="."):
    imports_fmtstr = f"from {rootmodname} import %s"
...
def _make_fromimport_str(from_imports, rootmodname="."):
    ...
    def _pack_fromimport(tup):
        ...
        from_module_str = f"from {rootmodname}.{name} import ("
