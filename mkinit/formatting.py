...
def _insert_autogen_text(modpath, initstr, interface=False):
    ...
    logger.debug(f"inserting initstr into: {init_fpath!r}")
...
def _find_insert_points(lines):
    ...
    for lineno, line in enumerate(lines):
        ...
        if explicit_flag and line.strip().startswith("# </AUTOGEN_INIT>"):
            print(f"[mkinit] FOUND END TAG ON LINE {lineno}: {line}")
            endline = lineno
        ...
        if not explicit_flag:
            ...
            if line.strip().startswith(implicit_patterns):
                ...
                try:
                    ...
                    startline = skipto
                    print(f"SKIPTO = {skipto!r}")
                except ValueError:
                    print(f"NOT ON A PS1 LINE KEEP {startline}")
                except IndexError:
                    print(f"LAST LINE MOVING TO END {startline}")
                    startline = endline
            else:
...
def _initstr(
    ...
    if options["lazy_loader"]:
        ...
        if explicit_exports:
            submodules = submodules
            print(f"submodules = {submodules!r}")
        else:
            submodules = set()
    ...
    if options.get("with_all", True):
        ...
        exports_repr = [f"'{e}'" for e in sorted(exposed_all)]
...
def _packed_rhs_text(lhs_text, rhs_text):
    ...
    newline_prefix = " " * 4 if len(lhs_text) > max_width * 0.7 else " " * len(lhs_text)
...
def _make_fromimport_str(from_imports, rootmodname=".", indent=""):
    ...
    def _pack_fromimport(tup):
        ...
        normname = rootmodname + name if name.startswith(".") else name

        if len(fromlist) > 0:
            lhs_text = indent + f"from {normname} import ("
            rhs_text = ", ".join(fromlist) + ",)"
            packstr = _packed_rhs_text(lhs_text, rhs_text)
        else:
            packstr = ""
    ...
    return textwrap.dedent(from_str)
