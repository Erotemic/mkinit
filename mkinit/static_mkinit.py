...
def ensure_options(given_options=None, _hack={}):
    """
    ...
    Defaults are:
        ...
        mods (bool): if False, then modules will not be imported.
            (Default: True)
        relative (bool): if True, then `.` is used instead of modname
            (Default: False)

    """
    # _hack lets use be compatible with older API
    if given_options is None:
        given_options = {}
    default_options = {
        'attrs': _hack.get('attrs', True),
        'mods': True,
        'relative': False,
    }
...
def autogen_init(modpath_or_name, imports=None, attrs=True, use_all=True,
    ...
    """
    options = ensure_options(options, _hack=dict(attrs=attrs))
