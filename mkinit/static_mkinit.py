...
def autogen_init(
    ...
    """
    logger.info(f"Autogenerating __init__ for modpath_or_name={modpath_or_name}")
...
def _parse_user_declarations2(init_fpath):
    ...
    Example:
        ...
        >>> assert user_decl["__submodules__"] == [
        ...     "aliased value that does not overwrite"
        ... ]
    ...
    class UserDeclarationVisiter(ast.NodeVisitor):
        ...
        def visit_ClassDef(self, node):
            ...
            if node.name == "__module_properties__":
                ...
                self.user_decl["module_property_names"] = self._classmethods[node.name]
        ...
        def visit_Assign(self, node):
            for target in node.targets:
                ...
                if target_id in user_decl_name_to_main:
                    ...
                    try:
                        self.user_decl[main_id] = static._parse_static_node_value(
                            node.value
                        )  # type: ignore
...
def _extract_attributes(modpath=None, source=None, respect_all=True):
    ...
    Example:
        ...
        >>> modpath = util_import.modname_to_modpath(
        ...     "mkinit.util.util_diff", hide_init=False
        ... )
        >>> _extract_attributes(modpath)
    """
    if source is None:
...
def _static_parse_imports(
    ...
    Example:
        ...
        >>> tup = _static_parse_imports(
        ...     modpath, submodules=submodules, external=external
        ... )
        >>> modname, submodules, from_imports = tup
        >>> print("modname = {!r}".format(modname))
        >>> print("submodules = {!r}".format(submodules))
    ...
    def _lookup_extractable_attrs(rel_modname):
        ...
        try:
            ...
            warnings.warn(f"Failed to parse module {rel_modname!r}, ex = {ex!r}")
    ...
    if external:
        for ext_modname in external:
            ...
            try:
                ...
                warnings.warn(f"Failed to parse {ext_modname!r}, ex = {ex!r}")
            else:
